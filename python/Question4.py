...
def question04(v, c, mc):
    ...
    vector= np.zeros(len(c_np))
    try:
        vector[df[df['c_sum']<mc].index]=1
    except:
        pass
    
    x = cvxpy.Variable(len(c_np), boolean=True)
    x.value = vector
    constraint = [c_np * x <= mc]
    obj = cvxpy.Maximize(v_np * x)
    
    prob = cvxpy.Problem(obj,constraint,)
    answer = prob.solve(solver=cvxpy.ECOS_BB,max_iters=10, mi_abs_eps=1e-5,mi_rel_eps=1e-4)
